...
def classify_frames(imagefile,labelfile,windowsize,framesize = -1):
    ...
    # HEADER of file
    HEADER,IDS,LABELS,COORDS = nedc_digipath_lib.nedc_fileio.parse_annotations(labelfile)
    NIL = phg.Nil()
    NIL.open(imagefile)
    ...
    if framesize == -1:
        ...
        for i in range(len(COORDS)):
            shapes.append(nedc_digipath_lib.nedc_geometry.generate_polygon(COORDS[i]))
        ...
        #
        window_list = nedc_digipath_lib.nedc_fileio.svs_windows_to_RGBA(imagefile,labels = outlabels,coords = outcoords,window_frame = [framesize,framesize],name = "file")
        
        # iterate through each frame and append that to a file for training
        #
        nedc_digipath_lib.nedc_fileio.RGBA_to_dct(window_list,imagefile)
