...
class ImageDataset(Dataset):
    ...
    @ray.remote
    def readFrames(self, i, annotation_file, image_file):
        ...
        header, ids, labels, region_coordinates = fileio_tools.parseAnnotations(annotation_file)
        ...
        print(f"RGBs retrieved in image {i}")
        return (window_RGBs, window_labels)
        
        
    @ray.remote
    def countFrames(self,i,image_file):
        image_reader = nedc_image_tools.Nil()
        image_reader.open(image_file)
        width, height = image_reader.get_dimension()
        frames = math.ceil(width/self.frame_size[0]) * math.ceil(height/self.frame_size[1])
        print(f"Counted frames in image {i}")
        return frames
    ...
    def __init__(self, image_files,
        ...
        windows_process = [self.readFrames.options(num_cpus=cpus_per_batch,
                                                   num_gpus=gpus_per_batch,
                                                   memory = memory).remote(self, i, annotation_file, image_file) for i, (annotation_file, image_file) in enumerate(zip(self.annotation_files[0:self.image_cache_size], self.image_files[0:image_cache_size]))]
        ...
        print(f"Counting lines in {len(self.image_files)}")

        ray_process = [self.countFrames.options(num_cpus=.05,
                                                num_gpus=0,
                                                memory=.1 * 1024 * 1024).remote(self, i, image_file) for i, image_file in enumerate(self.image_files)]
        frames_per_image = ray.get(ray_process)
        windows = ray.get(windows_process)
        self.RGBs = []
        self.labels = []
        for RGBs, labels in windows:
            self.RGBs.extend(RGBs)
            self.labels.extend(labels)
        #frames_per_image = [self.countFrames(i, image_file) for i, image_file in enumerate(self.image_files)]
    ...
    def __getitem__(self, idx):
        ...
        if len(self.labels) == 0:
            ...
            self.current_process = [self.readFrames.options(num_cpus=self.cpus_per_batch,
                                                            ...
                                                            memory=self.memory).remote(self, i, annotation_file, image_file) for i, (annotation_file, image_file) in enumerate(zip(self.annotation_files[self.current_file_index:self.current_file_index+to_add], self.image_files[self.current_file_index:self.current_file_index+to_add]))]
            self.RGBs = []
            self.labels = []
            for RGBs, labels in windows:
                self.RGBs.extend(RGBs)
...
class two_dimensional_convolutional_neural_network(torch.nn.Module):

    def __init__(self, number_of_classes:int, model_output_path:str,
                 layer_01_convolution_output_channels:int=32,
                 layer_01_convolution_kernel_size:int=3,
                 layer_01_max_pooling_kernel_size:int=3,
        ...
        super(two_dimensional_convolutional_neural_network, self).__init__()
        ...
        print(shape) 
        fully_connected_input_size = shape[0] * shape[1]

        print(fully_connected_input_size, shape)
    ...
    def frameCollate(self, batch):
        labels = []
        data = []
        for sample in batch:
            data.append(self.transform(sample['Data']))
            labels.append(sample['Label'])
        return data, labels
    ...
    def fit(self, image_files, annotation_files, batch_size = 4096):
        #if (self.device_type == "gpu"):
        #    data = data.to(self.device_type)
        #    labels = labels.to(self.device_type)
        ...
        dataset = ImageDataset(image_files, annotation_files, self.frame_size,
                               self.window_size, self.overlap_threshold, cpus_per_batch = 1,
                               memory = 2, image_cache_size=10)

        number_of_batches = math.ceil(len(dataset)/batch_size)
        dataloader = DataLoader(dataset, batch_size = batch_size, collate_fn=self.frameCollate)
        ...
        for epoch in range(self.number_of_epochs):
            ...
            for data, labels in dataloader:
                ...
                correct = sum([(predicted == actual) for predicted,actual in zip(predicted_classes,labels)])
                labels_correct+=correct
                total_labels+=total
                print(f"Batch {batch_number} of {number_of_batches}", flush=True)
                batch_number+=1
            torch.save(self, self.model_output_path + f"CNN{epoch}.pth")
            print(f'Epoch {epoch+1}/{self.number_of_epochs}, Loss: {loss.item()}, Accuracy: {labels_correct/total_labels}', flush=True)
    ...
    def dataToTensor(self, data):
        ...
        tensors = tensors.to(self.device_type)
        return tensors
    
    # takes in a list of labels where each label is represented by a string
    def labelToTensor(self, labels):
        int_labels = [label_order[label].value for label in labels]
        return torch.tensor(int_labels).to(self.device_type)
...
def main():
    ...
    dataloader = DataLoader(dataset, batch_size=1, collate_fn=tmp_collate)

    for data, label in dataloader:
        print(data, label)'''
    
    CNN = two_dimensional_convolutional_neural_network(9,"./models/")

    device = CNN.getDevice()
    
    CNN = torch.nn.DataParallel(CNN)
    
    CNN = CNN.to(device)

    CNN.module.fit(image_files, annotation_files, batch_size = 10)
